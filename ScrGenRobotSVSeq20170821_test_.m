  function ScrGenRobotSVSeq20170821_test_
  
fid=fopen('ScrGenSeq3Round_20170818_test.txt','w');      %结合 FlowPath20170818 液路图
...
% ADN 4 %%    代号19，运动到试剂位下位
  ...
  function AnnealingSegPrimer(fid, para)
        %% Annealing
        o(fid,'LOG Annealing ');
...
%     阀切换至RA RB
  ...
  function RARBRinsingNeedle(fid)
  
   for i=1:2            %%在needle清洗前排完溢流池的液体，重新充满干净液体，重复2次。
    o(fid,'SV[3] 1');   %%  打开排废液
    o(fid,'WAIT 5000'); %%  排废液，5秒钟
    o(fid,'SV[4] 0');  %%  切换到稀释液入口0
    o(fid,'SV[3] 0');  %%  切换到溢流池
    ...
    MixingPump(fid, 300, 3000, 300, 3000);
      ...
      %% 针RA、RB在清洗池内各吸吐五次
          ...
          o(fid,'SV[1] 0');%ly修改
          o(fid,'SV[2] 0'); %ly修改
     for k  = 1:5   %%test 1:5->1:1 吸吐各一次测试
          o(fid,'RV_7970 P 5');
